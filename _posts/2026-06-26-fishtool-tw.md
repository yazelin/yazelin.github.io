...
這篇記的是把它重建成 Astro + Vercel 現代站的過程。重點不是「新框架比較潮」,我們來看兩個真正花時間解決的問題。
...
## 第二個問題:AI 自作主張把品牌名編錯了,做驗收時抓出來改回去

早期有一版,首頁的品牌字標從舊站原本的「FishTool.tw」被 AI 換成自己編的「RED SEA TACKLE」,大概是想說英文化聽起來比較潮。我沒交代要換名字,它自己就決定了。

驗收時看到不對,改回了「FishTool.tw」。不是因為英文字標不好看,是那根本不是這家店的名字。重建一個老店的網站,長相可以現代化,但招牌不是 AI 能決定要不要換的東西,那是客戶的品牌資產。

我覺得這條教訓比圖片代理那條更值得記:技術問題有客觀對錯,AI 判斷失誤還能靠測試抓;但品牌這種東西沒有測試能擋,做得再漂亮,不是本人要的,就是做錯了,全靠甲方自己驗收把關。
...
我覺得這個切分不是先規劃好的架構美學,是先跑出真實的 build 時間、看哪裡拖慢了,我們才決定哪些頁面該先算好、哪些讓伺服器現算。
