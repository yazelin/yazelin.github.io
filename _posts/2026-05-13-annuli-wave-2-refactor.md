...
> - 🌱 **Mori 桌面身體**：[yazelin.github.io/mori-desktop](https://yazelin.github.io/mori-desktop/)（Annuli 是她的後端記憶服務）
...
[2026-04-09 的 Annuli MVP]({% post_url 2026-04-09-annuli-mvp %}) 是 222 行 `app.py`、樹輪隱喻、`/sleep` 反思、單一全域 persona。MVP 階段就清楚會有撐不住的點：單一使用者、沒有意圖分類、沒有 keyword 搜尋、`/sleep` 是 destructive 操作。
...
到 2026-05 中，`engine.py` 從 222 行膨脹到 **2489 行**、`admin.py` 1468 行、`adapters/cli.py` 391 行。單檔過大、責任不清的後果在維護面浮現：找一個 function 要捲很久、改 bug 容易踩到別的 feature。這已經從 monolithic 變成 monolithic mud ball。

2026-05-13 開始 Wave 2 重構。
...
**後續發展**：Annuli 會跟 [mori-desktop Phase 3]({% post_url 2026-05-18-mori-desktop-hey-mori %}) 整合，讓 Mori 的桌面身體真正使用反思引擎。Wave 3 的 ring-based reflection 也會慢慢落地。模組化讓這些改動只需要動到對應的子模組，不需要動整個系統。
