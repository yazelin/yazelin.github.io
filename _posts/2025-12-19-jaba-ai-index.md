---
layout: post
title: "Jaba AI 技術分享系列：完整目錄"
subtitle: "從專案整合到 AI 安全、從架構設計到 LINE Bot 進階開發"
date: 2025-12-19
categories: [Index]
...
| 主題 | 文章 | 說明 |
|------|------|------|
| LINE Bot 基礎 | [LINE Bot 開發入門：以呷爸點餐系統為例]({% post_url 2025-12-09-line-bot-guide %}) | Messaging API、Webhook 基礎 |
| Claude AI 基礎 | [架構選擇：Claude CLI 整合與對話設計]({% post_url 2025-12-11-claude-ai-part1-architecture %}) | CLI vs API、對話管理 |
| Docker 基礎 | [Docker 基礎概念與常用指令]({% post_url 2025-12-13-docker-basics %}) | 容器化基礎 |
| FastAPI + Socket.IO | [後端架構：FastAPI + Socket.IO 雙向通訊]({% post_url 2025-12-11-web-terminal-part2-socketio %}) | 即時通訊基礎 |
...
| # | 文章 | 重點 |
|---|------|------|
| 1 | [從 jaba + jaba-line-bot 到 jaba-ai：專案整合實戰]({% post_url 2025-12-20-jaba-ai-part1-integration %}) | 整合動機、架構決策、踩過的坑 |
...
| # | 文章 | 重點 |
|---|------|------|
| 1 | [Repository Pattern 實作：讓資料層乾淨分離]({% post_url 2025-12-21-jaba-ai-part2-repository %}) | 分層架構、Generic CRUD、依賴注入 |
| 2 | [事件隊列設計：解決 Socket.IO 與 DB Commit 順序問題]({% post_url 2025-12-21-jaba-ai-part3-event-queue %}) | ContextVar、事務化通知、一致性保證 |
...
| # | 文章 | 重點 |
|---|------|------|
| 1 | [Prompt Injection 防護實作]({% post_url 2025-12-22-jaba-ai-part4-prompt-injection %}) | 輸入消毒、規則檢測、安全日誌 |
| 2 | [AI 日誌系統：追蹤每一次對話]({% post_url 2025-12-22-jaba-ai-part5-ai-logging %}) | 對話記錄、Token 統計、異常分析 |
...
| # | 文章 | 重點 |
|---|------|------|
| 1 | [LINE Bot v3 SDK + FastAPI 非同步整合]({% post_url 2025-12-23-jaba-ai-part6-linebot-v3 %}) | SDK v3 差異、非同步處理、簽章驗證 |
| 2 | [LINE 群組權限設計：從申請到審核的完整流程]({% post_url 2025-12-23-jaba-ai-part7-group-permission %}) | 申請流程、審核機制、狀態機設計 |
...
| # | 文章 | 重點 |
|---|------|------|
| 1 | [自然語言點餐：從使用者輸入到訂單建立]({% post_url 2025-12-24-jaba-ai-part8-natural-language %}) | 意圖解析、上下文理解、跟單功能 |
| 2 | [菜單圖片 AI 辨識：上傳照片自動建立菜單]({% post_url 2025-12-24-jaba-ai-part9-menu-ocr %}) | 圖片處理、Claude Vision、結構化輸出 |
...
| 專案 | 文章 | 說明 |
|------|------|------|
| Jaba 看板 | [呷爸 jaba - 透過 LINE 群組輕鬆訂餐]({% post_url 2025-12-08-jaba %}) | 第一代：純看板系統 |
| Jaba LINE Bot | [Jaba LINE Bot - 讓點餐對話進入 LINE]({% post_url 2025-12-09-jaba-line-bot %}) | 第二代：LINE Bot 整合 |
...
- [Alembic](https://alembic.sqlalchemy.org/) - 資料庫遷移工具
