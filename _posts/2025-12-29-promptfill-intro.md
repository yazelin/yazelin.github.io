...
這是我新加入的功能，解決「同一份資料 + 不同範本 = 不同輸出」的需求。

**問題場景**：你有一份會議記錄，想分別：
- 用「摘要範本」產生重點摘要
- 用「翻譯範本」翻成英文
- 用「待辦範本」提取 action items

以前你得把會議記錄複製貼上三次。現在用資料來源功能：

1. 把會議記錄存成一個「資料來源」
2. 在範本中用 {% raw %}`{{__datasource__}}`{% endraw %} 佔位符
3. 選擇要套用的資料來源
4. 複製時自動替換成完整內容

在左側面板可以切換「詞庫」和「資料來源」兩個 Tab，資料來源支援：
- **大量文本**：適合存放會議記錄、文章、程式碼等長文
- **跨範本共用**：同一份資料來源可套用多個不同範本
- **截斷預覽**：編輯時只顯示前 100 字，避免畫面擁擠
...
新增 `{{__datasource__}}` 佔位符，可以引用儲存在左側面板的資料來源：

- 在左側面板新增「資料來源」Tab，與詞庫並列
- 支援新增、編輯、刪除資料來源
- 範本中使用 `{{__datasource__}}` 會渲染成下拉選單
- 複製時自動替換成完整的資料來源內容
- 資料來源選擇獨立於範本，切換範本時保持選中狀態

這個功能適合處理「同一份資料 + 不同範本」的場景，例如對同一份會議記錄分別做摘要、翻譯、提取待辦事項。
...
資料來源功能讓你在產生圖表時，可以附上參考資料讓 AI 有所依據：

1. 把專案的架構說明、操作文檔等存為「資料來源」
2. 在圖表 Prompt 範本中加入 {% raw %}`{{__datasource__}}`{% endraw %} 引用這份資料
3. AI 會根據參考資料產生更精準的圖表

這樣 Prompt 範本可以專注在圖表風格、格式的描述，而參考資料獨立管理，不用每次都重複貼上整份文檔。
