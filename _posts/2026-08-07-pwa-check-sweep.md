...
先講清楚這篇的性質：它是通則，是我這一整批站踩出來的總經驗，不是某一個專案的開發紀錄。所以下面每一條我都會標它是在哪個站上被踩到的，不然讀起來會像憑空的最佳實務。
...
每一個可能被單獨開啟或分享的頁都是入口。漫畫的章節頁、遊戲的攻略頁、每日一張圖的單張頁，每一頁都要各自掛 manifest、icon links，各自註冊 SW。

只掛首頁的話，從分享連結進來的人完全看不到安裝選項。他不會回報這件事，因為他根本不知道這個站可以裝。

這條同樣是一次修七個站：catime、gewu-jianghu-web、ipas-ai-quiz、line-chat-maker、line-sticker-studio、roll-formosa、token-unlimited-comic，順便補上 maskable 圖示跟 scope。修法不是「記得要掛」，是把這道轉換寫進建置或同步腳本，讓它自己發生。
...
`Vary` 那條也是格物教的。那站斷網時「圖都在、音樂不能播」，我一開始以為是音檔太大沒存進去，真因是 `cache.match` 的 `Vary` 比對對不上，全面加 `ignoreVary` 才好。同一個站還教了另一條：音檔走 Range 請求拿到的是 206，`Cache.put` 對 206 直接 throw，所以要擋掉它、另外抓一次完整檔補存。這兩條都是「本機測永遠測不到」的類型，也是檢查器現在會驗媒體能不能真的解碼的原因。
...
實測數字來自[格物江湖錄那個站]({% post_url 2026-07-27-gewu-jianghu-web %})：SHELL 跟 ASSET 原本共用一個快取名，而它每次部署都 bump，`activate` 就把 `assets/` 整包 33MB 刪掉重抓。量過的結果是改版後若瀏覽器 HTTP 快取已被清，實抓 28.86MB；分層之後同樣情境是 0.84MB。分層後 SHELL 只剩約 0.4MB。

省流量還只是附帶的。真正的問題是每次重寫 33MB 都在製造掉檔窗口：`cache.put` 失敗（配額不足、SW 被回收）是靜默的，排在最後、檔案最大的音檔最容易掉，於是使用者拿到的是「圖都在、音樂不能播」。

改成分層的那一版要一次性接手舊快取的資產，不然修好的那一次反而讓既有使用者再付一次全量。
